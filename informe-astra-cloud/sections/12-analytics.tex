...
Los datos operacionales de ASTRA CLOUD pertenecen a dominios diferentes: el catálogo se almacena en PostgreSQL, la información social en MySQL y las interacciones en MongoDB. Los contenedores de ingesta los extraen en archivos CSV y los depositan en Amazon S3. AWS Glue permite catalogar esas estructuras y Athena ejecuta SQL sobre la información consolidada, por lo que se pueden realizar análisis transversales sin trasladar carga de consulta a los microservicios transaccionales.

\section{Panel de analítica en la aplicación desplegada}
El cliente web expone la ruta \path{/analytics} para usuarios autenticados. Durante el recorrido de validación, el panel solicitó los indicadores de \path{/api/v1/dashboard/kpis} y las vistas de géneros, actores y clubes; los resultados se presentan sin ejecutar operaciones que modifiquen los datos.

La Figura~\ref{fig:analytics-dashboard} muestra el resultado observado. El panel concentra los conteos de películas, usuarios, clubes, salas e interacciones, junto con las tablas de géneros más vistos, actores populares y clubes activos. Esta vista conecta la API de analítica implementada con un consumo visible en la aplicación: los datos consolidados se traducen en indicadores que un administrador puede consultar desde la interfaz.

La misma evidencia permite identificar una limitación del despliegue actual: el campo \emph{Rating Promedio} aparece como \texttt{NaN}. Por ello, no se interpreta ese valor como una métrica válida en este informe; su corrección requiere revisar el dato retornado o su transformación en el cliente.
